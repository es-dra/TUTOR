    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Project':
        ...
        if "role_conversations" in data:
            data["role_conversations"] = [
                RoleMessage.from_dict(msg) 
                for msg in data["role_conversations"]
            ]
        # 处理标签字段（向后兼容）
        if "tags" not in data:
            data["tags"] = []
...
class ProjectManager:
    ...
    def update_project_tags(self, project_id: str, tags: List[str]) -> Optional[Project]:
        """更新项目标签"""
    ...
    def list_projects_by_tags(self, tags: List[str], match_all: bool = False) -> List[Project]:
        """按标签筛选项目"""
